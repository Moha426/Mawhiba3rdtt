...
import { CalendarDays, ChevronRight, ChevronLeft, Palmtree, Star, Flag, Cake, Sun, ClipboardCheck, Clock } from "lucide-react";
...
export default function CalendarPage() {
  ...
  const allDates = useMemo(() => {
    ...
    for (const a of assignments) dateSet.add(a.dueDate.split("T")[0]);
    for (const { date } of expandedEvents) dateSet.add(date);
        ...
        <div className="relative border-r-2 border-muted pr-6 mr-2 space-y-10 pb-12">
          ...
          {groups.map((group) => (
            ...
            >
              ...
              />
              <h2 className={`text-lg font-bold mb-3 flex items-center gap-2 flex-wrap ${group.past ? "text-muted-foreground" : "text-foreground"}`}>
                {group.label}
                {group.past && <Badge variant="secondary" className="text-xs rounded-full">فات موعدها</Badge>}
