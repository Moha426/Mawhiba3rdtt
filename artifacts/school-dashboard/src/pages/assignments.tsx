...
function autoScore(a: Assignment): number {
  ...
  // Score bands by urgency zone:
  // overdue (<0): -1000 + days (most overdue first)
  // today (0): 0 + typeW*10 + priorityW
  // this week (1-6): 100*days + typeW*10 + priorityW
  // later: 100*days + typeW*10 + priorityW  (same formula, naturally higher)
  if (daysUntilDue < 0) {
    return -1000 + daysUntilDue * 10 + typeW + priorityW; // most overdue first
  }
...
function sortAssignments(list: Assignment[], key: SortKey): Assignment[] {
  ...
  switch (key) {
    ...
    case "due-asc":
      return copy.sort((a, b) => parseISO(a.dueDate).getTime() - parseISO(b.dueDate).getTime());
    case "due-desc":
      return copy.sort((a, b) => parseISO(b.dueDate).getTime() - parseISO(a.dueDate).getTime());
    case "priority":
      return copy.sort((a, b) => (PRIORITY_WEIGHT[a.priority] ?? 9) - (PRIORITY_WEIGHT[b.priority] ?? 9));
    case "newest":
      return copy.sort((a, b) => parseISO(b.assignedDate).getTime() - parseISO(a.assignedDate).getTime());
    default:
      return copy;
  }
}
