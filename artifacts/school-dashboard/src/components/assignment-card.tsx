...
import {
  ...
  Download, ChevronDown, Folder, ListChecks, CalendarDays,
} from "lucide-react";
...
function parseAttachment(raw: string): AttachmentInfo {
  if (raw.startsWith("{")) {
    try {
      const parsed = JSON.parse(raw) as { url: string; label?: string; category?: string };
      const isFile = parsed.url.startsWith("/api/storage");
      const defaultLabel = isFile
        ? decodeURIComponent(parsed.url.split("/").pop() || "ملف")
        : (() => { try { return new URL(parsed.url).hostname.replace(/^www\./, ""); } catch { return "رابط"; } })();
      return {
        url: parsed.url,
      ...
      return { url: raw, label: "رابط", isFile: false };
    }
  }
  if (raw.startsWith("/api/storage")) {
    return {
      url: raw,
      label: decodeURIComponent(raw.split("/").pop() || "ملف"),
      isFile: true,
    };
  }
  try {
    const u = new URL(raw);
    return { url: raw, label: u.hostname.replace(/^www\./, ""), isFile: false };
  } catch {
    return { url: raw, label: "رابط", isFile: false };
  }
}
...
export function AssignmentCard({ assignment, completedIds = [], onToggleComplete }: AssignmentCardProps) {
  ...
  const formattedDate = format(dueDate, "EEEE، d MMMM yyyy", { locale: ar });

  const { isSignedIn } = useAuth();
  const isCompleted = completedIds.includes(assignment.id);
  const dueBadge = getDueBadge(dueDate, isCompleted);

  const checklistItems: string[] = (assignment as any).checklistItems || [];
  const attachments: AttachmentInfo[] = (assignment.attachmentLinks || []).map((a) => parseAttachment(typeof a === "string" ? a : JSON.stringify(a)));
          ...
          <div className="flex flex-col gap-2 mt-2">
            ...
            <div className="flex flex-wrap items-center text-xs text-muted-foreground gap-4 mt-1">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span>أُسندت: {format(parseISO(assignment.assignedDate), "d MMM", { locale: ar })}</span>
              </div>
