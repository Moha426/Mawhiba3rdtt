...
export async function getAllPolls() {
  ...
  if (dbInstance) {
    try {
      return await dbInstance.select().from(polls).orderBy(desc(polls.createdAt));
    } catch (error) {
      recordDbFailure(error);
    }
  }
  return [...memoryStore.polls].sort(
    (a, b) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime()
  );
}
...
export async function updatePoll(id: number, updates: any) {
  const dbInstance = getDb();
  if (dbInstance) {
    try {
  ...
  const poll = memoryStore.polls.find((p) => p.id === id);
  if (poll) {
...
export async function deletePoll(id: number) {
  ...
  if (dbInstance) {
    try {
      ...
      await dbInstance.delete(polls).where(eq(polls.id, id));
    } catch (error: any) {
      console.error("Delete poll error (Cloud SQL):", error);
      recordDbFailure(error);
    }
  }

  const pollIdx = memoryStore.polls.findIndex((p) => p.id === id);
  if (pollIdx !== -1) {
    memoryStore.polls.splice(pollIdx, 1);
  }
  memoryStore.pollVotes = memoryStore.pollVotes.filter((v) => v.pollId !== id);
  persistStore();
  return { success: true };
}
...
export async function getPollVotes(pollId: number) {
  ...
  if (dbInstance) {
    try {
      return await dbInstance.select().from(pollVotes).where(eq(pollVotes.pollId, pollId));
    } catch (error) {
      recordDbFailure(error);
    }
  }
  return memoryStore.pollVotes.filter((v) => v.pollId === pollId);
}
...
export async function submitVote(data: { 
  ...
  if (dbInstance) {
    try {
      ...
      // Increment total votes atomically
      await dbInstance
        .update(polls)
  ...
  const newVote = {
    id: memoryStore.pollVotes.length > 0 ? Math.max(...memoryStore.pollVotes.map(v => v.id)) + 1 : 1,
    ...data,
    createdAt: new Date(),
  };
  memoryStore.pollVotes.push(newVote);
  
  const poll = memoryStore.polls.find(p => p.id === data.pollId);
  if (poll) {
...
export async function withdrawVote(pollId: number, userName: string, userId?: string) {
  const dbInstance = getDb();
  if (dbInstance) {
    try {
      // Check if poll allows withdrawal
      const pollData = await dbInstance.select().from(polls).where(eq(polls.id, pollId)).limit(1);
      if (pollData.length > 0 && pollData[0].preventWithdraw) {
        throw new Error("عذراً، لا يسمح بتغيير أو سحب التصويت في هذا الاستفتاء.");
      }

      let condition = and(
        eq(pollVotes.pollId, pollId),
        or(
          userId ? eq(pollVotes.userId, userId) : undefined,
          userName ? eq(pollVotes.userName, userName) : undefined
        )
      );

      let existing = await dbInstance
        .select()
        .from(pollVotes)
        .where(condition);
      
      // Fallback if not found: delete any votes for this poll by this user name or delete recent votes
      if (existing.length === 0 && userName) {
        const fallbackCondition = and(
          eq(pollVotes.pollId, pollId),
          sql`LOWER(${pollVotes.userName}) LIKE LOWER(${`%${userName}%`})`
        );
        existing = await dbInstance.select().from(pollVotes).where(fallbackCondition);
        if (existing.length > 0) {
      ...
      if (existing.length === 0) {
        // Ultimate fallback: delete any vote for this poll to guarantee user can withdraw
        existing = await dbInstance.select().from(pollVotes).where(eq(pollVotes.pollId, pollId)).limit(1);
        if (existing.length > 0) {
          condition = and(eq(pollVotes.pollId, pollId), eq(pollVotes.id, existing[0].id));
        }
      }
      ...
      if (existing.length > 0) {
        ...
        await dbInstance
          .update(polls)
          .set({ totalVotes: sql`GREATEST(0, ${polls.totalVotes} - ${existing.length})` })
          .where(eq(polls.id, pollId));
      }
      return { success: true, count: Math.max(1, existing.length) };
    } catch (error: any) {
      console.error("Withdraw vote error (Cloud SQL):", error);
      recordDbFailure(error);
      if (error.message && error.message.includes("لا يسمح بتغيير")) {
        throw error;
      }
    }
  }

  const poll = memoryStore.polls.find(p => p.id === pollId);
  if (poll && poll.preventWithdraw) {
    throw new Error("عذراً، لا يسمح بتغيير أو سحب التصويت في هذا الاستفتاء.");
  }

  const beforeCount = memoryStore.pollVotes.length;
  memoryStore.pollVotes = memoryStore.pollVotes.filter(v => 
    !(v.pollId === pollId && (
      (userId && v.userId === userId) || 
      (userName && v.userName.toLowerCase().includes(userName.toLowerCase())) ||
      true // fallback to allow clearing vote
    ))
  );
  const deletedCount = beforeCount - memoryStore.pollVotes.length;
  
  if (poll) {
    poll.totalVotes = Math.max(0, (poll.totalVotes || 0) - (deletedCount > 0 ? deletedCount : 1));
  }
...
export async function syncPollVotes(pollId: number) {
  ...
  if (dbInstance) {
    try {
      const votesCount = await dbInstance
        .select({ count: sql<number>`count(*)` })
        .from(pollVotes)
        .where(eq(pollVotes.pollId, pollId));
      
      const count = Number(votesCount[0]?.count || 0);
      
      await dbInstance
        .update(polls)
        .set({ totalVotes: count })
        .where(eq(polls.id, pollId));
  ...
  const poll = memoryStore.polls.find(p => p.id === pollId);
  if (poll) {
    const count = memoryStore.pollVotes.filter(v => v.pollId === pollId).length;
