...
const LOCAL_STROKES_KEY = "talented_whiteboard_strokes_room_";

function getLocalRoomStrokes(roomId?: number): Stroke[] {
  try {
    const raw = localStorage.getItem(`${LOCAL_STROKES_KEY}${roomId ?? 0}`);
...
function saveLocalRoomStrokes(roomId: number | undefined, strokes: Stroke[]) {
  try {
    localStorage.setItem(`${LOCAL_STROKES_KEY}${roomId ?? 0}`, JSON.stringify(strokes));
    window.dispatchEvent(new CustomEvent("whiteboard_sync_event", {
      detail: { roomId: roomId ?? 0, strokes }
    }));
  } catch {}
}
...
export function Whiteboard({ roomId, myId, onShareSnapshot, className }: WhiteboardProps) {
  ...
  const [strokes,  setStrokes]  = useState<Stroke[]>(() => getLocalRoomStrokes(roomId));
  ...
  useEffect(() => {
    ...
    function onUp() {
      ...
      setStrokes(prev => {
        const next = [...prev, s];
        saveLocalRoomStrokes(roomId, next);
        return next;
      });
      ...
      if (roomId) {
        postStroke(roomId, encodeStroke(s)).then(dbId => {
          if (dbId != null) {
            ...
            setStrokes(prev => {
              const updated = prev.map(x => x.id === lid ? { ...x, dbId } : x);
              saveLocalRoomStrokes(roomId, updated);
              return updated;
            });
          }
        });
  ...
  /* ── Collaborative sync — poll for new persistent strokes and listen to local events ── */
  useEffect(() => {
    const handleSyncEvent = (e: any) => {
      if (e.detail?.roomId === (roomId ?? 0) && Array.isArray(e.detail?.strokes)) {
        setStrokes(e.detail.strokes);
      }
    };
    window.addEventListener("whiteboard_sync_event" as any, handleSyncEvent);

    if (!roomId) {
      return () => window.removeEventListener("whiteboard_sync_event" as any, handleSyncEvent);
    }

    /* On mount: load all existing strokes from server or fallback to local */
    fetchStrokes(roomId, 0).then(rows => {
      ...
      setStrokes(loaded);
      saveLocalRoomStrokes(roomId, loaded);
    ...
    const timer = setInterval(async () => {
      ...
      if (toAdd.length > 0) {
        setStrokes(prev => {
          ...
          saveLocalRoomStrokes(roomId, all);
    ...
    return () => {
      clearInterval(timer);
      window.removeEventListener("whiteboard_sync_event" as any, handleSyncEvent);
    };
  }, [roomId, myId]);
  ...
  const undo = useCallback(() => {
    setStrokes(s => {
      ...
      const next = s.filter(x => x.id !== own.id);
      saveLocalRoomStrokes(roomId, next);
      return next;
    });
  }, [roomId, myId]);
