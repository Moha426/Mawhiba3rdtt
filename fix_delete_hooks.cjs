...
code = code.replace(/mutate:\s*\(([^,]+),\s*opts\?:\s*any\)\s*=>\s*\{\s*(set[A-Za-z0-9_]+)\(\(prev\)\s*=>\s*prev\.filter\(\(a\)\s*=>\s*a\.id\s*!==\s*\1\)\);\s*opts\?\.onSuccess\?\.\(\);\s*\}/g,
(match, arg, setter) => {
...
code = code.replace(/mutateAsync:\s*async\s*\(([^)]+)\)\s*=>\s*(set[A-Za-z0-9_]+)\(\(prev\)\s*=>\s*prev\.filter\(\(a\)\s*=>\s*a\.id\s*!==\s*\1\)\)/g,
(match, arg, setter) => {
